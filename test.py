...
from model import MultiHeadAttention,TransformerBlock,LayerNormalization,Attention,Self_Attention,MultiHeadSelfAttention,AttentionWithContext

def predict(X_test, K_test, y_test, thred, para, weights, jsonFiles, h5_model, dir):
    ...
    for ii in range(0, len(weights)):
        ...
        load_my_model = load_model(h5_model_path,custom_objects={"MultiHeadAttention" : MultiHeadAttention,"TransformerBlock":TransformerBlock,"LayerNormalization":LayerNormalization,"Attention":Attention,"Self_Attention":Self_Attention(output_dim=300),"MultiHeadSelfAttention":MultiHeadSelfAttention,"AttentionWithContext":AttentionWithContext})
        print("Prediction is in progress")

        # 2.predict
        score = load_my_model.predict([X_test, K_test])
...
def test_my(test, para, model_num, dir):
    # step1: preprocessing
    test[2] = keras.utils.to_categorical(test[2])
    test[0], test[1], temp = catch(test[0], test[1], test[2])
    temp[temp > 1] = 1
    test[2] = temp
    ...
    # step2:predict
    predict(test[0], test[1], test[2], test[3], para, weights, jsonFiles, h5_model, dir)